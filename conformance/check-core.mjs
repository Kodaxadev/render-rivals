...
function extractBacktickBullets(source) {
  return [...source.matchAll(/^\s*-\s+`([^`]+)`[;.]?\s*$/gm)].map(
    (item) => item[1],
  );
}
...
async function checkRunState(root) {
  ...
  const specStates = extractBacktickBullets(
    section.slice(section.indexOf("`RunState`:")),
  );
...
async function checkApiContracts(root) {
  ...
  const spec13 = await readText(
    root,
    "spec/13-configuration-cli-and-local-api-contracts.md",
  );
  const spec17 = await readText(
    root,
...
async function checkUiPairingScope(root) {
  ...
  const text = await readText(root, file);
  const routeSection = extractSection(
    text,
...
async function checkFixtureDefinition(root) {
  ...
  if (windowsCase) {
    if (
      !Array.isArray(windowsCase.observedValues) ||
      windowsCase.observedValues.length < 2
    ) {
      issues.push(
        makeIssue(
          "DOC-WINDOWS-TARGET-DRIFT",
